...
public class ClientService {
    ...
    // Search Properties auditLog not needed
    ...
    // View Clicked Property (Property Details) auditLog not needed
    public PropertyDetailResponse viewClickedProperty(int propertyId) {
    ...
    public BookingResponse makeBooking(BookingRequest request) {
        ...
        if (property.getPropertyStatus() != PropertyStatus.AVAILABLE) {
            auditService.createAuditLog(ActionType.ERROR, "Tried to book property but Property is not available for " +
                    "booking, " +
                    "UserId: " + request.getUserId() + " PropertyId: " + request.getPropertyId());
            throw new InvalidOperationException("Property is not available for booking");
        }

        // Validate dates
        if (request.getCheckoutDate().isBefore(request.getCheckinDate())) {
            auditService.createAuditLog(ActionType.ERROR,"Property already booked for selected dates");
            throw new IllegalArgumentException("Checkout date cannot be in the past. Booking not done");
        }

        if (request.getCheckinDate().isBefore(LocalDate.now())) {
            auditService.createAuditLog(ActionType.ERROR,"CheckIn date cannot be in the past. Booking not done");
            throw new IllegalArgumentException("CheckIn date cannot be in the past");
        }
        ...
        booking.setIsBookingStatus(BookingStatus.BOOKED);

        Booking savedBooking = bookingRepository.save(booking);

        auditService.createAuditLog(ActionType.CREATE,"New booking added successfully");

        return convertToBookingResponse(savedBooking);
    }

    // View Bookings auditLog not needed
    public List<BookingResponse> viewBooking(int userId) {
...
//    }
    ...
    // Add Complaint for Booking added auditLog
    public ComplaintResponse addComplaintForBooking(ComplaintRequest request) {
        ...
        if(dateDifference >= 7){
            auditService.createAuditLog(ActionType.ERROR, "Client tried adding a complaint for a booking beyond the " +
                    "validity" +
                    " window. Booking ID: " + booking.getBookingId());
        ...
        auditService.createAuditLog(ActionType.CREATE,
                "New Complaint booked UserId: " + request.getUserId() + " BookingId " + request.getBookingId());
        return convertToComplaintResponse(savedComplaint);
    }

    // View Complaints by User auditLog not needed
    public List<ComplaintResponse> viewComplaints(int userId) {
    ...
    public boolean makePayment(PaymentRequest paymentRequest) {
        ...
        String url = "http://localhost:8081/transaction/transferByAccount";

        // Set headers
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        Optional<Property> tempProperty = propertyRepository.findById(paymentRequest.getPropertyId());
        if(tempProperty.isPresent()) {
            Property property = tempProperty.get();
            System.out.println("getPropertyId: " + property.getPropertyId());
            System.out.println("getPropertyName: " + property.getPropertyName());
            System.out.println("getPropertyAccountNumber: " + property.getPropertyAccountNumber());
            PaymentExternalServerRequest paymentExternalServerRequest = new PaymentExternalServerRequest((paymentRequest.getAccountNumber()).toString(), (property.getPropertyAccountNumber()).toString(), paymentRequest.getAmount(), "Payment for booking room from SpaceFinders by userId: " + paymentRequest.getUserId());
            System.out.println("Outbound Request is made now");
            // Create the HTTP request entity
            HttpEntity<PaymentExternalServerRequest> entity = new HttpEntity<>(paymentExternalServerRequest, headers);


            try {
                // Make POST request
                ResponseEntity<String> response = restTemplate.postForEntity(url, entity, String.class);
                System.out.println("Outbound Request done");
                // Handle response
                if (response.getStatusCode().is2xxSuccessful()) {
                    assert response.getBody() != null;
                    if((response.getBody().contains("Transaction Successful"))){
                        System.out.println("Payment successful: " + response.getBody());
                        return true;
                    }else{
                    ...
                    System.out.println("Payment failed: " + response.getStatusCode());
                    return false;
                }
